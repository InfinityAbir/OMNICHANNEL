...
/// <summary>
/// Same properties ChannelWebhookSecurityTests proves against a fake adapter, verified again here
/// against the real, production-registered WhatsAppChannelAdapter (Phase 7) — a provider-specific
/// signature bug wouldn't be caught by the generic pipeline tests alone.
/// </summary>
public class WhatsAppWebhookSecurityTests(TestWebApplicationFactory factory) : IClassFixture<TestWebApplicationFactory>
{
    ...
    public async Task Webhook_ForgedSignature_IsRejectedAndNeverPersisted()
    {
        ...
        var externalAccountId = $"waid-{Guid.NewGuid():N}";
        await agent.PutAsJsonAsync(new Uri("/api/v1/channels/whatsapp/account", UriKind.Relative), new SetChannelExternalAccountRequest(externalAccountId));

        const string bodyTemplate = """
        {"object":"whatsapp_business_account","entry":[{"id":"1","changes":[{"value":{"messaging_product":"whatsapp","metadata":{"phone_number_id":"__ACCOUNT__"},"messages":[{"from":"15551234567","id":"wamid.forged","timestamp":"1749416383","type":"text","text":{"body":"forged"}}]},"field":"messages"}]}]}
        """;
        ...
        request.Headers.Add("X-Hub-Signature-256", "sha256=" + new string('0', 64)); // well-formed but wrong
    ...
    public async Task Webhook_GenuineSignature_RoutesOnlyToConnectedTenant()
    {
        ...
        var externalAccountIdA = $"waid-{Guid.NewGuid():N}";
        await tenantA.PutAsJsonAsync(new Uri("/api/v1/channels/whatsapp/account", UriKind.Relative), new SetChannelExternalAccountRequest(externalAccountIdA));

        using var tenantB = factory.CreateClient();
        tenantB.UseBearer(await RegisterAsync(tenantB));
        var externalAccountIdB = $"waid-{Guid.NewGuid():N}";
        await tenantB.PutAsJsonAsync(new Uri("/api/v1/channels/whatsapp/account", UriKind.Relative), new SetChannelExternalAccountRequest(externalAccountIdB));

        const string bodyTemplate = """
        {"object":"whatsapp_business_account","entry":[{"id":"1","changes":[{"value":{"messaging_product":"whatsapp","metadata":{"phone_number_id":"__ACCOUNT__"},"messages":[{"from":"15551234567","id":"wamid.real1","timestamp":"1749416383","type":"text","text":{"body":"for tenant A only"}}]},"field":"messages"}]}]}
        """;
