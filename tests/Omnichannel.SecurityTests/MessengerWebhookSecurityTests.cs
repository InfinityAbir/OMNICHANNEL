...
/// <summary>PRD §68's "repeat external integration security checklist" — mirrors WhatsApp/InstagramWebhookSecurityTests against Messenger's own real production adapter.</summary>
public class MessengerWebhookSecurityTests(TestWebApplicationFactory factory) : IClassFixture<TestWebApplicationFactory>
{
    ...
    public async Task Webhook_ForgedSignature_IsRejectedAndNeverPersisted()
    {
        ...
        var externalAccountId = $"page-{Guid.NewGuid():N}";
        await agent.PutAsJsonAsync(new Uri("/api/v1/channels/messenger/account", UriKind.Relative), new SetChannelExternalAccountRequest(externalAccountId));

        const string bodyTemplate = """
        {"object":"page","entry":[{"id":"__ACCOUNT__","messaging":[{"sender":{"id":"1"},"recipient":{"id":"__ACCOUNT__"},"timestamp":1458692752478,"message":{"mid":"forged-mid","text":"forged"}}]}]}
        """;
    ...
    public async Task Webhook_GenuineSignature_RoutesOnlyToConnectedTenant()
    {
        ...
        var externalAccountIdA = $"page-{Guid.NewGuid():N}";
        await tenantA.PutAsJsonAsync(new Uri("/api/v1/channels/messenger/account", UriKind.Relative), new SetChannelExternalAccountRequest(externalAccountIdA));

        using var tenantB = factory.CreateClient();
        tenantB.UseBearer(await RegisterAsync(tenantB));
        var externalAccountIdB = $"page-{Guid.NewGuid():N}";
        await tenantB.PutAsJsonAsync(new Uri("/api/v1/channels/messenger/account", UriKind.Relative), new SetChannelExternalAccountRequest(externalAccountIdB));

        const string bodyTemplate = """
        {"object":"page","entry":[{"id":"__ACCOUNT__","messaging":[{"sender":{"id":"1"},"recipient":{"id":"__ACCOUNT__"},"timestamp":1458692752478,"message":{"mid":"real-mid-1","text":"for tenant A only"}}]}]}
        """;
...
file static class HttpClientExtensions3
{
