...
/// <summary>
/// The actual OpenAI-compatible chat-completions request/response logic (system prompt, JSON
/// schema, parsing, error mapping) — extracted so both <see cref="GroqAiProvider"/> (the
/// platform-default provider, IOptions-driven, unchanged since Phase 10) and
/// <see cref="OpenAiCompatibleProvider"/> (Phase 16, tenant-configured base URL/key/model) share
/// one implementation instead of duplicating it. Every OpenAI-compatible vendor (Groq, OpenAI
/// itself, Together, Fireworks, Mistral, DeepSeek, OpenRouter, self-hosted OpenAI-shim servers,
/// ...) speaks this same `/chat/completions` shape — see ADR-0027.
/// </summary>
internal static class OpenAiCompatibleChatClient
{
    ...
    public static async Task<AiCompletionResult> GenerateSuggestionAsync(
        HttpClient httpClient, string providerLabel, string baseUrl, string apiKey, string model,
        AiPromptContext context, CancellationToken cancellationToken)
    {
        ...
        var request = new ChatCompletionRequest(model, messages, new ResponseFormat("json_object"), 0.3);
        ...
        {
            throw new AiProviderException($"Network error calling {providerLabel} API: {ex.Message}", ex);
        }
        catch (TaskCanceledException ex) when (!cancellationToken.IsCancellationRequested)
        {
            throw new AiProviderException($"Timeout calling {providerLabel} API: {ex.Message}", ex);
        }

        var body = await response.Content.ReadAsStringAsync(cancellationToken);
        if (!response.IsSuccessStatusCode)
        {
            throw new AiProviderException($"{providerLabel} API returned {(int)response.StatusCode}: {body}");
        }
        ...
        {
            throw new AiProviderException($"{providerLabel} API returned an unparseable response.", ex);
        }

        var content = completion?.Choices?.FirstOrDefault()?.Message?.Content;
        if (string.IsNullOrWhiteSpace(content))
        {
            throw new AiProviderException($"{providerLabel} API returned no completion content.");
        }
